...
function studentData(firstName, lastName, age, marksArray, ...rest) {
  ...
  obj.getResult = function () {
    ...
    let totalSub = marksArray.length;
...
function massageArray(inputArray, categoriesDirectory, areas) {
  return inputArray.reduce((acc, item) => {

    let obj = {};
    obj.roductId = item.idMeal;

    acc.push(JSON.stringify(obj))

    // acc[item.idMeal] && acc[item.idMeal].push(item.idMeal)
